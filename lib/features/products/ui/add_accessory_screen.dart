...
class AddAccessoryScreen extends StatefulWidget {
  ...
  const AddAccessoryScreen({super.key, this.product});
...
class _AddAccessoryScreenState extends State<AddAccessoryScreen> {
            ...
            child: SingleChildScrollView(
              ...
              child: Form(
                ...
                child: Column(
                  ...
                  children: [
                    _buildSectionTitle('معلومات الملحقات'),
                    _buildTextField(
                      _nameController,
                      'اسم الملحق',
                      isRequired: true,
                    ),
                    _buildTextField(_barcodeController, 'الباركود (اختياري)'),
                    Row(
                      children: [
  ...
  void _autoSelectCategory(List<Category> allCategories) {
    ...
    final accCategories = allCategories.where((c) {
      final name = c.name.toLowerCase();
      return name.contains('ملحق') ||
          name.contains('إكسسوار') ||
          name.contains('accessory');
    }).toList();

    if (accCategories.isNotEmpty) {
      setState(() {
        _selectedCategoryId = accCategories.first.id;
      });
    }
  }
    ...
    return Row(
      children: [
        ...
        ElevatedButton(
          ...
          child: Text(
            widget.product == null ? 'حفظ الملحق' : 'حفظ التعديلات',
            style: GoogleFonts.cairo(
              fontWeight: FontWeight.bold,
              fontSize: 14.sp,
            ),
